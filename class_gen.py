...
def gen_dao():
    global manager_class_name, manager_package, manager_file_path
    # 生成DAO
    print('生成DAO...')
...
def gen_service():
    ...
    service_class += SERVICE_CLASS_TEMP % (
        table_name, table_comment, current_user, current_date, service_class_name, base_service.split(POINT).pop(),
        manager_class_name,
        class_name)
...
please input 0-2:  ''')
    ...
    funcMap = {
        0: [gen_model, gen_dao, gen_service],
        1: [gen_model],
        2: [gen_model, gen_dao]
    }
    for i, func in enumerate(funcMap[gen_mode]):
        func()
    print('OK')
